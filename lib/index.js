...
var _PointerDriver_states, _PointerDriver_nextPointerID;
class PointerDriver {
    constructor() {
        _PointerDriver_states.set(this, new Map());
        _PointerDriver_nextPointerID.set(this, 0);
    }
    registerPointer() {
        return __classPrivateFieldSet(this, _PointerDriver_nextPointerID, +__classPrivateFieldGet(this, _PointerDriver_nextPointerID) + 1);
    }
    unregisterPointer(pointer) {
        for (const [root, state] of __classPrivateFieldGet(this, _PointerDriver_states)) {
            // Queue leave event if unregistered pointer was assigned to root
            if (state.pointer === pointer) {
                state.pointer = null;
                if (state.hovering) {
                    state.eventQueue.push(new Leave(root.lastFociCapturers.get(exports.FocusType.Pointer)));
                }
    ...
    movePointer(root, pointer, xNorm, yNorm, pressing) {
        const state = __classPrivateFieldGet(this, _PointerDriver_states).get(root);
    ...
    onEnable(root) {
        ...
        __classPrivateFieldGet(this, _PointerDriver_states).set(root, {
            eventQueue: [],
            pointer: null,
            pressing: false,
            hovering: false,
    ...
    onDisable(root) {
        ...
        __classPrivateFieldGet(this, _PointerDriver_states).delete(root);
    }
    update(root) {
        const state = __classPrivateFieldGet(this, _PointerDriver_states).get(root);
        if (typeof state === 'undefined')
...
_PointerDriver_states = new WeakMap(), _PointerDriver_nextPointerID = new WeakMap();
