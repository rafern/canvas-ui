...
(function (ThemeProperty) {
    ...
    ThemeProperty[ThemeProperty["InputTextFlexRatio"] = 22] = "InputTextFlexRatio";
    ThemeProperty[ThemeProperty["InputTextMinWidth"] = 23] = "InputTextMinWidth";
    ThemeProperty[ThemeProperty["InputTextMinAscent"] = 24] = "InputTextMinAscent";
    ThemeProperty[ThemeProperty["InputTextMinDescent"] = 25] = "InputTextMinDescent";
    ThemeProperty[ThemeProperty["BlinkRate"] = 26] = "BlinkRate";
    ThemeProperty[ThemeProperty["CursorPadding"] = 27] = "CursorPadding";
    ThemeProperty[ThemeProperty["CursorThickness"] = 28] = "CursorThickness";
    ThemeProperty[ThemeProperty["ScrollBarThickness"] = 29] = "ScrollBarThickness";
...
function Variable(Base, defaultValue) {
    return class Variable extends Base {
        constructor() {
            ...
            this.callback = null;
            // The current value
            this._value = defaultValue; // XXX protected
        }
        get value() {
            return this._value;
        }
...
var _blinkStart, _blinkWasOn, _cursorPos, _cursorOffset, _cursorOffsetDirty, _editingEnabled, _hideText;
// FIXME protected members were turned public due to a declaration emission bug:
// https://github.com/Microsoft/TypeScript/issues/17744
class TextInput extends Labelable(Variable(FlexWidget, '')) {
    ...
    moveCursorTo(index) {
        ...
        __classPrivateFieldSet(this, _cursorPos, Math.min(Math.max(index, 0), this._value.length));
    ...
    insertText(str) {
        ...
        this.value = this._value.substring(0, __classPrivateFieldGet(this, _cursorPos)) + str + this._value.substring(__classPrivateFieldGet(this, _cursorPos));
    ...
    deleteText(delta) {
        ...
        // cursor
        if (delta > 0)
            this.value = this._value.substring(0, __classPrivateFieldGet(this, _cursorPos)) + this._value.substring(__classPrivateFieldGet(this, _cursorPos) + delta);
        else if (delta < 0) {
            // NOTE, still checking if delta < 0 so that nothing is done if
            // delta is 0
            this.value = this._value.substring(0, __classPrivateFieldGet(this, _cursorPos) + delta) + this._value.substring(__classPrivateFieldGet(this, _cursorPos));
    ...
    handleEvent(event, _width, _height, root) {
        ...
        if (event instanceof PointerEvent) {
            ...
            // Get mobile-friendly text input if available
                ...
                root.getTextInput(this._value).then((newValue) => {
                    if (newValue === null)
                        return;
...
_blinkStart = new WeakMap(), _blinkWasOn = new WeakMap(), _cursorPos = new WeakMap(), _cursorOffset = new WeakMap(), _cursorOffsetDirty = new WeakMap(), _editingEnabled = new WeakMap(), _hideText = new WeakMap();
